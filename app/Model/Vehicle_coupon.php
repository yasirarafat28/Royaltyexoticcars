...
class Vehicle_coupon extends Model
{
    ...
    public static function CheckCouponDiscountAmount($vehicle_id){
        $VoucherDiscount = 0;
        $finalDiscount = 0;

        $dt = date('Y-m-d');
        $item = Vehicle::find($vehicle_id,$TotalCarted=0);
        if (!$item){
            return 0;
        }
        $voucher_data = Vehicle_coupon::whereRaw('"' . $dt . '" between `start_at` and `end_at`')->where('code', $item->voucher_code)->first();
        if ($voucher_data){
